...
export class DataCache<T> {
	...
	getCacheKey(options: FetchOptions): string {
		const sortKey =
			options.sorting && options.sorting.length > 0
		...
		const columnFilterKey =
			options.columnFilters && options.columnFilters.length > 0
		...
		const customKeys = Object.entries(options)
			.filter(
				([key]) =>
					!["sorting", "pagination", "columnFilters", "globalFilter"].includes(
						key,
					),
			)
